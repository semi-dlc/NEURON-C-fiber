: nattxs_noise4.mod is a transient ttx-sensitive Na+ current from
: Sheets et al 2007
: with noise implemented from
: Guhler 2013 

: and PEPD mutation implemented
: (+20 mV depolarizing shift of the fast inactivation)

NEURON {
       SUFFIX nattxs_noise_1000p					:name of mechanism
...
PROCEDURE rates(Vm (mV)) {
    ...
    ah = A_ah/(1+exp(((Vm- 20)+shift+B_ah)/C_ah)) :+20 mV depolarizing shift of the fast inactivation
    as = 0.00003 + A_as/(1+exp((Vm+shift+B_as+Tshift)/C_as))
    bm = A_bm/(1+exp((Vm+shift+B_bm)/C_bm))
    bh = -0.00283 + A_bh/(1+exp(((Vm-20)+shift+B_bh)/C_bh)) :+20 mV depolarizing shift of the fast inactivation
